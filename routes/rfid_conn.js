    ...
    pool.getConnection((err, connection) => {
        ...
        if (Number(term) === 1 || Number(term) === 2) {
            connection.query(parentQuery, [rfid], (err, result) => {
                ...
                if (result.length != 0) {
                    connection.query(prevQuery, [rfid, plis, loc, result[0]["RLID"], term], (err, result) => {
                        connection.release();
                        if (err) {
                            return res.status(500).json({ message: 'Failed to insert data', error: err });
                        }
            ...
            connection.query(newQuery, [rfid, plis, loc, term], (err, result) => {
                connection.release();
                if (err) {
                    return res.status(500).json({ message: 'Failed to insert data', error: err });
                }
